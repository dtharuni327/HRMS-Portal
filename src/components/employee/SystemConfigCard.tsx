...
export default function SystemConfigCard() {
  ...
  useEffect(() => {
    // Fetch system config from localStorage
  ...
  // If no config is saved, show empty state with message
  if (!config || !Object.values(config).some((val) => val)) {
    return (
      <div className="rounded-[20px] border border-white/10 bg-white/5 p-6 backdrop-blur">
        <div className="mb-4">
          ...
          <p className="mt-2 text-sm text-white/60">
            Organization's attendance and timing policies
          </p>
        </div>

        <div className="rounded-lg border border-white/10 bg-white/5 p-4 text-center">
          <p className="text-sm text-white/70">
            <span className="font-semibold">No configuration available yet.</span> Super Admin will set up your organization's attendance and timing policies soon.
          </p>
        </div>
      </div>
    );
  ...
  const configItems = [
    ...
    {
      label: "Overtime Rate",
      value: config.overtimeRate ? `₹${config.overtimeRate}/hr` : "--",
    ...
    <div className="rounded-[20px] border border-white/10 bg-white/5 p-6 backdrop-blur">
        ...
        {configItems.map((item, index) => (
          ...
          >
            <div className="mb-3 flex items-center justify-between">
              ...
              <div
                className={`rounded-lg bg-gradient-to-br ${item.color} p-1.5`}
              >
      ...
      <div className="mt-5 rounded-lg border border-white/10 bg-white/5 p-3 text-xs text-white/70">
        <p>
          ...
          organization's Super Admin. Please refer to your HR team for any updates.
        </p>
      </div>
    </div>
  );
