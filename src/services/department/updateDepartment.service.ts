...
export const updateDepartmentService = async (data: any, user: any) => {
  const { department_id, department_name, description, status } = data;
  
  const result = await updateDepartmentRepository(department_id, { department_name, description, status }, user);
