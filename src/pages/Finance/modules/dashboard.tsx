...
const FinanceDashboardModule: FC = () => {
  ...
  const handleSendReminder = () => {
    alert('Payout reminder sent to the finance review team.');
  };

  const handleAddNote = () => {
    if (!note.trim()) {
      alert('Please enter a finance note before saving.');
      return;
    }

    alert(`Finance note saved: ${note.trim()}`);
    setNote('');
  };
  ...
  return (
    <div className="space-y-6">
      <SparkCard className="rounded-[2rem] border border-emerald-100 bg-[#effcf5] p-6 text-slate-800 shadow-[0_18px_45px_rgba(15,23,42,0.12)] backdrop-blur-xl">
        <p className="text-[10px] uppercase tracking-[0.35em] text-emerald-700">Quick actions</p>
        <h3 className="mt-3 text-xl font-black text-slate-900">Finance actions</h3>
        <p className="mt-2 text-sm text-slate-600">Use these controls to export payroll insights, send payout reminders, and save internal finance notes.</p>
        <div className="mt-4 flex flex-wrap gap-3">
          <button type="button" onClick={handleExportSummary} className="rounded-2xl bg-slate-900 px-4 py-2 text-sm font-semibold text-white hover:bg-slate-800">Export payroll summary</button>
          <button type="button" onClick={handleSendReminder} className="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Send payout reminder</button>
        </div>
        <label className="mt-4 block text-[12px] uppercase tracking-[0.18em] text-slate-700">
          Finance note
          <textarea value={note} onChange={(e) => setNote(e.target.value)} rows={3} className="mt-1 w-full rounded-2xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 outline-none" placeholder="Add an approval note, compliance update, or payout reminder." />
        </label>
        <button type="button" onClick={handleAddNote} className="mt-3 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm font-semibold text-emerald-800 hover:bg-emerald-100">Save finance note</button>
      </SparkCard>
