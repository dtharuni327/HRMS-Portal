...
const enrichEmployeeData = (emp: Employee): EmployeeWithContact => ({
  ...
  manager: emp.dept === 'Tech' ? 'Rahul Sharma' : emp.dept === 'Design' ? 'Ananya Rao' : 'Priya Singh',
});
...
> = ({ employees }) => {
  ...
  const departments = Array.from(new Set(enrichedEmployees.map(e => e.dept))).sort();
  ...
  <div className="space-y-6 animate-in slide-in-from-bottom-4 p-8">
    ...
    <div className="grid grid-cols-1 gap-6">
      ...
      {departments.map((dept, index) => {
          ...
          const deptEmployees = enrichedEmployees.filter(e => e.dept === dept);
