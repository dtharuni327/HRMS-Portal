...
const attendanceData = [ 
  { day: 'Mon', present: 80, wfh: 8, leave: 2, vibe: 90 },
  { day: 'Tue', present: 85, wfh: 6, leave: 3, vibe: 88 },
  { day: 'Wed', present: 82, wfh: 10, leave: 1, vibe: 94 },
  { day: 'Thu', present: 90, wfh: 5, leave: 2, vibe: 92 },
  { day: 'Fri', present: 88, wfh: 7, leave: 1, vibe: 96 },
  { day: 'Sat', present: 40, wfh: 5, leave: 5, vibe: 85 },
  { day: 'Sun', present: 35, wfh: 4, leave: 6, vibe: 80 },
];
...
const DashboardModule: FC<DashboardModuleProps> = ({
  ...
  const celebrations = [
    { name: 'Ananya Rao', type: 'Birthday', date: '05-19', note: 'Wish her a happy birthday today!' },
    { name: 'Vikram Seth', type: 'Anniversary', date: '06-01', note: 'Work anniversary coming soon.' }
  ];
  ...
  const monthCalendarDays = useMemo(() => {
    ...
    // Build live attendance data for the chart: override today's values from `attendanceStatus`
    const liveAttendanceData = useMemo(() => {
      const labels = DAY_LABELS;
      const todayLabel = labels[currentTime.getDay()];
      ...
      return attendanceData.map(d => {
        if (d.day === todayLabel) {
          ...
          return { ...d, present, wfh, leave };
        }
        return d;
      });
    }, [attendanceData, attendanceStatus, currentTime]);
  ...
  const todayEvents = useMemo(
    () => celebrations.filter((event) => event.date === todayMonthDay),
    [celebrations, todayMonthDay]
  );
        ...
        >
          ...
          <div className="flex flex-col gap-2 ml-3">
            ...
            <button
              onClick={() =>
                setWfhRequests(
                  wfhRequests.filter(r => r.id !== req.id)
                )
              }
            ...
            <button
              ...
              className="p-2 bg-rose-100 text-rose-600 rounded-xl"
            >
              <XCircle size={16} />
            </button>
      ...
      <SparkCard className="w-full p-8 bg-[#f2e8ff] border border-violet-100 rounded-[28px] shadow-[0_16px_45px_rgba(0,0,0,0.10)]">
        ...
        <div className="h-80 w-full flex items-center justify-center">
          <ResponsiveContainer width="100%" height="100%">
            {(() => {
              const counts = [
                { name: 'Tech', value: employees.filter(e => e.dept === 'Tech').length },
                { name: 'Design', value: employees.filter(e => e.dept === 'Design').length },
                { name: 'Admin', value: employees.filter(e => e.dept === 'Admin').length }
              ];
              const COLORS = ['#7c3aed', '#a78bfa', '#c7d2fe'];
              return (
                <PieChart>
                  <Pie data={counts} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={90} innerRadius={40} paddingAngle={4}>
