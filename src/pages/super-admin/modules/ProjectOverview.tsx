  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-4">
        <div className="rounded-lg bg-[#0f2440] p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs text-white/70">Total Projects</div>
        ...
        <div className="rounded-lg bg-[#0f2440] p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs text-white/70">Ongoing</div>
        ...
        <div className="rounded-lg bg-[#0f2440] p-4 text-white">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs text-white/70">Delayed</div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="col-span-2 rounded-lg bg-[#0f2440] p-4 text-white">
              ...
              <tbody>
                {mockProjects.map((p) => (
                  <tr key={p.id} className="border-t border-white/6">
                    <td className="px-3 py-3 text-sm text-white/80">{p.id}</td>
                    <td className="px-3 py-3 text-sm">{p.name}</td>
                    <td className="px-3 py-3 text-sm">{p.client}</td>
                    ...
                    <td className="px-3 py-3 text-sm text-white/80">{p.assignedEmployees.join(", ")}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="rounded-lg bg-[#0f2440] p-4 text-white">
          ...
          <div className="space-y-3">
            {Object.entries(clientMap).map(([client, projects]) => (
              <div key={client} className="flex items-center justify-between rounded border border-white/6 p-3">
                <div>
                  <div className="text-sm font-semibold">{client}</div>
                  <div className="text-xs text-white/70">{projects.length} project(s)</div>
                </div>
                <div className="text-sm text-white/80">{projects.map((p) => p.status).join(" • ")}</div>
              </div>
            ))}
          </div>
        </div>
