...
export const useProjectStore = create<ProjectStore>((set) => ({
  ...
  addAssignment: (projectName: string, member: Member) =>
    set((state) => ({
      assignments: {
        ...state.assignments,
        [projectName]: [...(state.assignments[projectName] ?? []), member],
      },
    })),
  removeAssignment: (projectName: string, memberName: string) =>
    set((state) => ({
      assignments: {
        ...state.assignments,
        [projectName]: (state.assignments[projectName] ?? []).filter(
          (member) => member.name !== memberName
        ),
      },
    })),
  setAssignmentsForProject: (projectName: string, members: Member[]) =>
    set((state) => ({
      assignments: {
        ...state.assignments,
        [projectName]: members,
