...
const ApprovalsModule: FC<ApprovalsModuleProps> = ({
  ...
  const applyLeaveStatusLocally = (leaveId: number, status: 'Approved' | 'Rejected', rejectionReason?: string) => {
    setLeaveData((prev) =>
      prev.map((leave) =>
        leave.id === leaveId
          ? {
  ...
  const applyWfhStatusLocally = (requestId: number, status: 'Approved' | 'Rejected', rejectionReason?: string) => {
    setWfhRequests((prev) =>
      prev.map((request) =>
  ...
  const syncWfhStatus = async (requestId: number, status: 'APPROVED' | 'REJECTED') => {
    const request = wfhRequests.find((item) => item.id === requestId);

    if (!request?.employeeId) {
      return;
    }

    await hrmsApi.updateWfhStatus({
  ...
  const confirmRejectLeave = async (leaveId: number) => {
    ...
    applyLeaveStatusLocally(leaveId, 'Rejected', reason);
  ...
  const confirmRejectWfh = async (requestId: number) => {
    ...
    try {
      await syncWfhStatus(requestId, 'REJECTED');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unable to reject WFH request.';
      alert(message);
      return;
    }

    applyWfhStatusLocally(requestId, 'Rejected', reason);
  ...
  const approveLeave = async (leaveId: number) => {
    ...
    applyLeaveStatusLocally(leaveId, 'Approved');
  };
  ...
  const approveWfhRequest = async (requestId: number) => {
    try {
      await syncWfhStatus(requestId, 'APPROVED');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Unable to approve WFH request.';
      alert(message);
      return;
    }

    applyWfhStatusLocally(requestId, 'Approved');
  };
