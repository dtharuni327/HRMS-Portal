    ...
    const loadApprovals = async () => {
      try {
        const [leaves, wfh] = await Promise.all([
          hrmsApi.getLeaves(),
          hrmsApi.getAllWfhRequests(),
        ]);

        if (Array.isArray(leaves) && leaves.length > 0) {
          setLeaveData(
            leaves.map((leave: any, index: number) => ({
              id: Number(leave.LeaveId ?? leave.leave_id ?? index + 1),
              employeeId: String(leave.Emp_id ?? leave.emp_id ?? ""),
              employee:
                leave.EmployeeName ??
                leave.name ??
                leave.employee ??
                `Employee ${leave.Emp_id ?? index + 1}`,
              type: leave.LeaveType ?? leave.leave_type ?? leave.type ?? "Casual",
              days: Number(leave.TotalDays ?? leave.days ?? 1),
              startDate: String(leave.FromDate ?? leave.from_date ?? leave.startDate ?? "").slice(0, 10),
              endDate: String(leave.ToDate ?? leave.to_date ?? leave.endDate ?? "").slice(0, 10),
              reason: leave.Reason ?? leave.reason ?? "",
              status:
                String(leave.Status ?? leave.status ?? "Pending")
                  .replace(/^\w/, (c: string) => c.toUpperCase()) as LeaveData["status"],
            })),
          );
        }

        if (Array.isArray(wfh) && wfh.length > 0) {
          setWfhRequests(
            wfh.map((request: any, index: number) => ({
              id: Number(request.Id ?? request.id ?? index + 1),
              employeeId: String(request.Emp_id ?? request.emp_id ?? ""),
              name:
                request.EmployeeName ??
                request.name ??
                `Employee ${request.Emp_id ?? index + 1}`,
              type: "WFH",
              reason: request.Reason ?? request.reason ?? "",
              date: String(request.FromDate ?? request.from_date ?? request.date ?? "").slice(0, 10),
              startDate: String(request.FromDate ?? request.from_date ?? "").slice(0, 10),
              endDate: String(request.ToDate ?? request.to_date ?? "").slice(0, 10),
              status:
                String(request.Status ?? request.status ?? "Pending")
                  .replace(/^\w/, (c: string) => c.toUpperCase()) as RequestItem["status"],
            })),
          );
        }
      } catch (error) {
        console.warn("Unable to load live approvals data for manager dashboard.", error);
      }
    };
