...
export default function UserRoleManagement() {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      try {
        const employees = await superAdminApi.getEmployees();
        const mappedUsers: User[] = employees.map((employee, index) => ({
          id: employee.id || `EMP-${index + 1}`,
          name: employee.name,
          email: employee.email,
          role: employee.role,
          status: employee.status === "inactive" ? "Inactive" : "Active",
          salaryPackage: "Not available",
        ...
        console.warn("Unable to load employees from backend, using local data.", error);
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const toggleUserStatus = async (id: string) => {
    ...
    try {
      await superAdminApi.updateEmployee(id, {
        ...target,
        status: nextStatus === "Inactive" ? "inactive" : "active",
      } as any);
    } catch (error) {
      console.warn("Employee status update failed in backend, keeping UI update local.", error);
    }
    ...
    <div>
      {isLoading && (
        <div className="mb-4 rounded-2xl border border-sky-400/20 bg-sky-500/10 px-4 py-3 text-sm text-sky-100">
          Loading employee data from backend...
        </div>
      )}
